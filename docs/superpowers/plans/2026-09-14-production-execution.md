...
- WIP is an explicit ledger, not a derived number: good output of a non-final operation is received there, and the next operation draws what it needs automatically when it reports production, FIFO from the costed lots. A manual transfer stays available for recording a physical hand-over. What an operation may never do is run ahead of the one before it.

...
func RenderPOPDF(order Order, includePrices bool) ([]byte, error) {
	...
	pdf.Rect(12, 7, 186, 24, "D")
	pdf.Line(12, 14, 198, 14)
	pdf.SetXY(15, 9)
	pdf.SetFont(pdfFontFamily, "B", 8)
	pdf.CellFormat(180, 4, pdfCompanyName(order.CompanyName), "", 0, "L", false, 0, "")
	pdf.SetXY(15, 17)
	pdf.SetFont(pdfFontFamily, "B", 17)
	pdf.CellFormat(105, 8, "PURCHASE ORDER", "", 0, "L", false, 0, "")
	pdf.SetFont(pdfFontFamily, "B", 11)
	pdf.CellFormat(75, 6, order.PONumber, "", 1, "R", false, 0, "")
	pdf.SetXY(123, 24)
...
func writeKeyValue(pdf *fpdf.Fpdf, key, value string, maxLines int) {
	...
	x, y := pdf.GetX(), pdf.GetY()
	pdf.Rect(x, y, keyWidth, rowHeight, "D")
	pdf.Rect(x+keyWidth, y, valueWidth, rowHeight, "D")
	pdf.SetFont(pdfFontFamily, "B", 9)
	pdf.SetXY(x+2, y)

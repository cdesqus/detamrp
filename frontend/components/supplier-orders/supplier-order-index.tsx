...
export function SupplierOrderIndex() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [response, supplierResponse] = await Promise.all([fetch(`/api/purchase-orders?${new URLSearchParams({ search })}`, { credentials: 'include' }), fetch('/api/master-data/suppliers?limit=200', { credentials: 'include' })]);
  ...
  return <section className="module-index supplier-order-index">
    ...
    <div className="table-frame"><table><thead><tr><th>PO Number</th><th>Supplier</th><th>Order Date</th><th>Expected Date</th><th>Status</th><th>Total</th><th>Created By</th><th>Action</th></tr></thead><tbody>
      {loading ? <tr><td colSpan={8}><div className="table-empty" role="status">Loading...</div></td></tr> : error ? <tr><td colSpan={8}><div className="table-empty" role="alert"><strong>Could not load supplier orders</strong><span>{error}</span><button className="table-action" onClick={load}>Retry</button></div></td></tr> : items.length === 0 ? <tr><td colSpan={8}><div className="table-empty" role="status"><strong>No supplier orders yet</strong><span>Create order to start.</span></div></td></tr> : items.map(order => <tr key={order.id}><td>{order.poNumber}</td><td>{order.supplierName ?? '—'}</td><td>{date(order.orderDate)}</td><td>{date(order.expectedDeliveryDate)}</td><td><span className="status-pill">{order.status.replaceAll('_', ' ')}</span></td><td>{order.currency} {formatDecimal(order.totalAmount)}</td><td>{order.createdBy?.displayName ?? '—'}</td><td><button className="table-action" aria-label={`Open ${order.poNumber}`} onClick={() => router.push(`/supplier-orders/${order.id}`)}>Open</button></td></tr>)}
    </tbody></table></div>
  </section>;
}

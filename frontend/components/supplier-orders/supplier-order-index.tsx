...
type Order = { id: string; poNumber: string; supplierId: string; supplierName?: string; orderDate: string; expectedDeliveryDate: string; status: string; totalAmount?: string; currency: string; createdBy?: { displayName?: string }; documents?: { deliveryNoteId: string; deliveryNoteNumber: string; kanbanCount: number; issuedAt: string } | null };
...
function DocumentLink({ href, label, children }: DocumentLinkProps) {
  return <a className="supplier-order-document-link" href={href} target="_blank" rel="noopener noreferrer" aria-label={label}>{children}</a>;
}
...
export function SupplierOrderIndex({ permissions }: Props = {}) {
  ...
  return <section className="module-index supplier-order-index">
    ...
    <div className="table-frame"><table><thead><tr><th>PO Number</th><th>Supplier</th><th>Order Date</th><th>Expected Date</th><th>Status</th>{canViewPrices && <th>Total</th>}<th>PO PDF</th><th>DN PDF</th><th>Kanban Labels</th><th>Created By</th><th>Action</th></tr></thead><tbody>
      {loading ? <tr><td colSpan={columnCount}><div className="table-empty" role="status">Loading...</div></td></tr> : error ? <tr><td colSpan={columnCount}><div className="table-empty" role="alert"><strong>Could not load supplier orders</strong><span>{error}</span><button className="table-action" onClick={load}>Retry</button></div></td></tr> : items.length === 0 ? <tr><td colSpan={columnCount}><div className="table-empty" role="status"><strong>No supplier orders yet</strong><span>Create order to start.</span></div></td></tr> : items.map(order => {
        const documentsAvailable = order.status === 'APPROVED' && order.documents;
        return <tr key={order.id}><td>{order.poNumber}</td><td>{order.supplierName ?? '—'}</td><td>{date(order.orderDate)}</td><td>{date(order.expectedDeliveryDate)}</td><td><span className="status-pill">{order.status.replaceAll('_', ' ')}</span></td>{canViewPrices && <td>{order.currency} {formatDecimal(order.totalAmount)}</td>}<td><DocumentLink href={`/api/purchase-orders/${order.id}/documents/po.pdf`} label={`Open PO PDF for ${order.poNumber}`}>PO PDF</DocumentLink></td><td>{documentsAvailable ? <DocumentLink href={`/api/purchase-orders/${order.id}/documents/delivery-note.pdf`} label={`Open DN PDF for ${order.poNumber}`}>DN PDF</DocumentLink> : '—'}</td><td>{documentsAvailable ? <DocumentLink href={`/api/purchase-orders/${order.id}/documents/kanban-labels.pdf`} label={`Open Kanban labels PDF for ${order.poNumber}`}>Labels PDF</DocumentLink> : '—'}</td><td>{order.createdBy?.displayName ?? '—'}</td><td><div className="supplier-order-row-actions"><button ref={element => { openTriggers.current[order.id] = element; }} className="table-action" aria-label={`Open ${order.poNumber}`} onClick={() => router.push(`/supplier-orders/${order.id}`)}>Open</button>{canEditDraft && order.status === 'DRAFT' && <div className="supplier-order-row-menu"><button ref={element => { actionTriggers.current[order.id] = element; }} className="supplier-order-row-menu-trigger" aria-label={`Actions for ${order.poNumber}`} aria-expanded={menuOrderId === order.id} aria-controls={menuOrderId === order.id ? `draft-actions-${order.id}` : undefined} onClick={() => setMenuOrderId(current => current === order.id ? '' : order.id)}>...</button>{menuOrderId === order.id && <div id={`draft-actions-${order.id}`} data-testid={`draft-actions-${order.id}`} className="supplier-order-row-menu-popover supplier-order-row-menu-popover--upward"><button onClick={() => openCancellation(order)}>Cancel Draft</button></div>}</div>}</div></td></tr>;
      })}
    </tbody></table></div>

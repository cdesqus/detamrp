  ...
  return <section className="module-index supplier-order-index">
    ...
    <div className="table-frame"><table><thead><tr><th className="transaction-number">No.</th><th>Actions</th><th>Documents</th><th>Status</th><th>PO Number</th><th>Supplier</th><th>Order Date</th><th>Expected Date</th>{canViewPrices && <th>Total</th>}<th>Sage No.</th><th>Created By</th></tr></thead><tbody>
      {loading ? <tr><td colSpan={columnCount}><div className="table-empty" role="status">Loading...</div></td></tr> : error ? <tr><td colSpan={columnCount}><div className="table-empty" role="alert"><strong>Could not load supplier orders</strong><span>{error}</span><button className="table-action" onClick={load}>Retry</button></div></td></tr> : items.length === 0 ? <tr><td colSpan={columnCount}><div className="table-empty" role="status"><strong>No supplier orders yet</strong><span>Create order to start.</span></div></td></tr> : pagedItems(items, page, pageSize).map((order, index) => {
        ...
        return <tr key={order.id}><td className="transaction-number">{(page - 1) * pageSize + index + 1}</td><td><div className="supplier-order-row-menu"><button ref={element => { actionTriggers.current[order.id] = element; openTriggers.current[order.id] = element; }} className="compact-menu-trigger" aria-label={`Actions for ${order.poNumber}`} aria-expanded={menuOrderId === order.id} onClick={() => { setDocsOrderId(''); setMenuOrderId(value => value === order.id ? '' : order.id); }}>Action <span>⌄</span></button>{menuOrderId === order.id && <div id={`draft-actions-${order.id}`} data-testid={`draft-actions-${order.id}`} className="supplier-order-row-menu-popover row-menu-list"><button onClick={() => router.push(`/supplier-orders/${order.id}`)}>Open Detail</button><button disabled={!canEditDraft || order.status !== 'DRAFT'} onClick={() => router.push(`/supplier-orders/${order.id}`)}>Edit Order</button><button disabled={!canSubmit || order.status !== 'DRAFT'} onClick={() => void sendApproval(order)}>Send to Approval</button><button disabled>Send to Supplier</button><div className="row-menu-divider" /><button className="row-menu-danger" disabled={!canEditDraft || order.status !== 'DRAFT'} onClick={() => openCancellation(order)}>Cancel Draft</button></div>}</div></td><td><div className="supplier-order-row-menu"><button className="compact-menu-trigger" aria-label={`Documents for ${order.poNumber}`} aria-expanded={docsOrderId === order.id} onClick={() => { setMenuOrderId(''); setDocsOrderId(value => value === order.id ? '' : order.id); }}>Docs <span>⌄</span></button>{docsOrderId === order.id && <div className="supplier-order-row-menu-popover row-menu-list"><DocumentLink href={`/api/purchase-orders/${order.id}/documents/po.pdf`} label={`Purchase Order PDF for ${order.poNumber}`} />{documentsAvailable ? <DocumentLink href={`/api/purchase-orders/${order.id}/documents/delivery-note.pdf`} label={`Delivery Note PDF for ${order.poNumber}`} /> : <button disabled>Delivery Note PDF</button>}{labelsAvailable ? <DocumentLink href={`/api/purchase-orders/${order.id}/documents/kanban-labels.pdf`} label={`Kanban Labels PDF for ${order.poNumber}`} /> : <button disabled>Kanban Labels PDF</button>}</div>}</div></td><td><OrderStatusBadge status={order.status} /></td><td>{order.poNumber}</td><td>{order.supplierName ?? '—'}</td><td>{date(order.orderDate)}</td><td>{date(order.expectedDeliveryDate)}</td>{canViewPrices && <td>{formatMoney(order.totalAmount, order.currency)}</td>}<td>{order.sagePurchaseOrderNumber || '—'}</td><td>{order.createdBy?.displayName ?? '—'}</td></tr>;
      })}

...
export function ReceivingIndex() {
  ...
  return <section className="module-index receiving-index">
    ...
    {open&&<><button className="crud-scrim" aria-label="Close receiving form" onClick={closeModal}/><div className="crud-modal receiving-create-modal" role="dialog" aria-modal="true" aria-label="New receiving"><div className="crud-modal-heading"><div><strong>New receiving</strong><span>Scan the DN attached to the physical shipment.</span></div><button aria-label="Close receiving form" onClick={closeModal}>×</button></div><form onSubmit={create}><div className="crud-fields receiving-create-fields"><label className="receiving-scan-field">Scan or Type DN Number<input aria-label="Scan or Type DN Number" value={value} onChange={event=>setValue(event.target.value)} autoComplete="off" autoFocus/><small>Scan the Delivery Note barcode or enter the number, then press Enter.</small></label>{error&&<p className="form-error" role="alert">{error}</p>}</div><div className="crud-actions"><button type="button" onClick={closeModal}>Cancel</button><button className="primary-button" disabled={!value.trim()||busy}>{busy?'Validating...':'Continue'}</button></div></form></div></>}
  </section>;
}

...
export function ApprovalInbox() {
  ...
  return <section className="module-index approval-inbox">
    ...
    {hasSnapshot ? <nav className="approval-pagination" aria-label="Approval pagination"><span aria-live="polite">{rangeStart}–{rangeEnd} of {total}</span><div><button aria-label="Previous approval page" disabled={offset === 0 || loading} onClick={() => changePage(offset - pageLimit)}>Prev</button><button aria-label="Next approval page" disabled={offset + pageLimit >= total || loading} onClick={() => changePage(offset + pageLimit)}>Next</button></div></nav> : null}

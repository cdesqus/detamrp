...
export function SalesOrderForm() {
  ...
  async function submit(event:FormEvent) {
    ...
    try {
      const response = await fetch('/api/sales-orders',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({customerId,orderDate,deliveryDate,customerPoReference,notes,lines})});
  ...
  return <section className="module-index"><div className="page-title-row"><div><h1>New Sales Order</h1><p className="muted">Select a customer and the finished goods they ordered. Prices follow the FG master and cannot be edited here.</p></div><button aria-label="Close new sales order" className="table-action" onClick={()=>router.push('/sales-orders')}>×</button></div>
    <form onSubmit={submit} className="crud-modal crud-modal--wide" style={{position:'static',transform:'none',margin:0,width:'100%',maxWidth:980}}>{error&&<p className="form-error">{error}</p>}<div className="crud-fields"><label><span>Customer *</span><select aria-label="Customer *" required value={customerId} onChange={event=>setCustomerId(event.target.value)}><option value="">Select...</option>{customers.map(customer=><option key={customer.id} value={customer.id}>{customer.code} — {customer.name}</option>)}</select></label><label><span>Order Date *</span><input type="date" required value={orderDate} onChange={event=>setOrderDate(event.target.value)}/></label><label><span>Planned Delivery Date</span><input type="date" min={orderDate} value={deliveryDate} onChange={event=>setDeliveryDate(event.target.value)}/></label><label><span>Customer PO Reference</span><input value={customerPoReference} onChange={event=>setCustomerPoReference(event.target.value)}/></label><label><span>Notes</span><textarea value={notes} onChange={event=>setNotes(event.target.value)}/></label></div>
